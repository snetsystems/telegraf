...
[[inputs.diskio_ext]]
  ## Device names to include in collection. Wildcards are supported.
  ## If empty, all devices are collected.
  ## ex. device_include = ["sda", "sdb", "vd*", "/dev/disk/by-id/nvme-eui.00123deadc0de123"]
  # device_include = ["*"]

  ## Device names to exclude from collection. Wildcards are supported.
  ## Exclusion is applied after inclusion: device_include -> device_exclude -> collect
  # device_exclude = ["loop*", "ram*", "sr*", "fd*"]
...
The `mount_path` tag also relies on `HOST_PROC` to correctly resolve the
host's mount points from within the container.
...
- diskio_ext
  - tags:
    ...
    - mount_path (mount point, Linux only)
    - wwid (World Wide Identifier, Linux only)
  - fields:
...
```sql
SELECT non_negative_derivative(last("io_time"),1ms) FROM "diskio_ext" WHERE time > now() - 30m GROUP BY "host","name",time(60s)
```
...
```sql
SELECT non_negative_derivative(last("weighted_io_time"),1ms) from "diskio_ext" WHERE time > now() - 30m GROUP BY "host","name",time(60s)
```

## Example Output

```text
diskio_ext,mount_path=/,name=sda1 merged_reads=0i,reads=2353i,writes=10i,write_bytes=2117632i,write_time=49i,io_time=1271i,weighted_io_time=1350i,read_bytes=31350272i,read_time=1303i,iops_in_progress=0i,merged_writes=0i 1578326400000000000
diskio_ext,mount_path=/var/log,name=centos/var_log reads=1063077i,writes=591025i,read_bytes=139325491712i,write_bytes=144233131520i,read_time=650221i,write_time=24368817i,io_time=852490i,weighted_io_time=25037394i,iops_in_progress=1i,merged_reads=0i,merged_writes=0i 1578326400000000000
diskio_ext,name=sda write_time=49i,io_time=1317i,weighted_io_time=1404i,reads=2495i,read_time=1357i,write_bytes=2117632i,iops_in_progress=0i,merged_reads=0i,merged_writes=0i,writes=10i,read_bytes=38956544i 1578326400000000000
```
