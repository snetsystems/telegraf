...
func Parse(acc telegraf.Accumulator, buf []byte) error {
	...
	for i := range s.Gpu {
		...
		common.SetIfUsed("float", fields, "gpu_power_draw", gpu.GpuPowerReadings.InstantPowerDraw)
		...
		common.SetIfUsed("float", fields, "gpu_power_limit", gpu.GpuPowerReadings.PowerLimit)
