...
func checkAbsentFields(t *testing.T, fields []string, acc *testutil.Accumulator) {
	for _, field := range fields {
		ok := acc.HasField("http_response_ext", field)
		require.False(t, ok)
	}
}
...
func checkAbsentTags(t *testing.T, tags []string, acc *testutil.Accumulator) {
	for _, tag := range tags {
		ok := acc.HasTag("http_response_ext", tag)
		require.False(t, ok)
	}
}
...
func checkFields(t *testing.T, fields map[string]interface{}, acc *testutil.Accumulator) {
	...
	for key, field := range fields {
		switch v := field.(type) {
		case int:
			value, ok := acc.IntField("http_response_ext", key)
			require.True(t, ok)
			require.Equal(t, field, value)
		case float64:
			value, ok := acc.FloatField("http_response_ext", key)
			require.True(t, ok)
			require.InDelta(t, field, value, testutil.DefaultDelta)
		case string:
			value, ok := acc.StringField("http_response_ext", key)
			require.True(t, ok)
			require.Equal(t, field, value)
		case nil:
			ok := acc.HasField("http_response_ext", key)
			require.True(t, ok)
		default:
...
func checkTags(t *testing.T, tags map[string]interface{}, acc *testutil.Accumulator) {
	for key, tag := range tags {
		switch v := tag.(type) {
		case string:
			ok := acc.HasTag("http_response_ext", key)
			require.True(t, ok)
			require.Equal(t, tag, acc.TagValue("http_response_ext", key))
		case nil:
			ok := acc.HasTag("http_response_ext", key)
			require.True(t, ok)
		default:
...
func TestAsyncExecutionTime(t *testing.T) {
	...
	// Configure multiple targets pointing to the same delay endpoint
	h := &HTTPResponse{
		Log: testutil.Logger{},
		URLs: []string{
			ts.URL + "/twosecondnap",
	...
	// Since we sleep for 2 seconds internally, the overall execution time should be slightly over 2s.
	// If it ran synchronously, it would take ~6 seconds.
	require.True(t, elapsed < time.Second*5, "Expected asynchronous gather to take less than 5 seconds, took %s", elapsed)
}
